    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            while step < self.num_batches:
                ...
                region_features, cnn_code = image_encoder(ave_img.cuda())
                ...
                fake_imgs, _, mu, logvar = netG(noise, sent_emb, words_embs, mask, cap_lens,region_features)
            ...
            for _ in range(1):  # (cfg.TEXT.CAPTIONS_PER_IMAGE):
                ...
                for step, data in enumerate(self.data_loader, 0):
                    ...
                    if R_count >= 6000:
                        ...
                        np.random.shuffle(R)
                        for i in range(10):
                            sum[i] = np.average(R[i * 600:(i + 1) * 600 - 1])
...
def get_ave_img(img_path):
    img = Image.open('/root/DM-GAN/ave_imgs/ave_img.jpg').convert('RGB')
    image_transform = transforms.Compose([transforms.Scale(256)])
    img = image_transform(img)
    norm = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
    ret = norm(img)
    imgs=torch.zeros(cfg.TRAIN.BATCH_SIZE,3,256,256)
