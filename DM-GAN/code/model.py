...
def conv3(in_planes, out_planes, bias=False):
    "3x3 convolution without padding"
    return nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=1, padding=0, bias=bias)

def conv5x5(in_planes, out_planes, bias=False):
    "3x3 convolution without padding"
    return nn.Conv2d(in_planes, out_planes, kernel_size=5, stride=1, padding=0, bias=bias)
...
class ACM(nn.Module):
    def __init__(self,in_channel, out_channel):
        ...
        t = int(abs((math.log(out_channel, 2) + 1) / 2))
        k_size = t if t % 2 else t + 1
        self.conv_small = conv1x1(in_channel, out_channel,bias=False)
        self.conv_middle = conv3(in_channel, out_channel,bias=False)
        self.conv_big = conv5x5(in_channel, out_channel,bias=False)
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.conv=nn.Conv1d(1,1,kernel_size=3,padding=1)#self.conv = nn.Conv1d(1, 1, kernel_size=k_size, padding=(k_size - 1) // 2, bias=False) 
        self.sigmoid=nn.Sigmoid()

    def ex(self,x):
        x=x.squeeze(-1).permute(0,2,1) #squeeze(-1)最后一维为1则去掉[8,128,1,1]->[8,128,1] permute(0,2,1)交换最后两维位置[8,128,1]->[8,1,128]
        x=self.conv(x) #对原通道进行卷积进行跨通道交互
        x=self.sigmoid(x) #bs,1,128
        x=x.permute(0,2,1).unsqueeze(-1) #[bs,1,128]->[bs,128,1]->[bs,128,1,1]
        return x
    def forward(self, x, seg_imgs):
        #x:ngf/8 x 32 x 32[8, 256, 32, 32]  [8,128,64,64]
        #seg_imgs:[8, 128, 32, 32]          [8,64,64,64]
        out_code_small = self.conv_small(seg_imgs)
        out_code_middle = self.conv_middle(seg_imgs)
        out_code_big = self.conv_big(seg_imgs)

        #全局平均池化
        out_code_small=self.avg_pool(out_code_small)
        out_code_middle=self.avg_pool(out_code_middle)
        out_code_big=self.avg_pool(out_code_big)
        x_small=self.ex(out_code_small)
        x_middle=self.ex(out_code_middle)
        x_big=self.ex(out_code_big)

        #通道级加法
        out_code=x_small+x_middle+x_big
        OutPutMF(out_code,x)
        return x*out_code.expand_as(x),out_code

def OutPutMF(out_code,x):
    for ib in range(cfg.TRAIN.BATCH_SIZE):
        out_code1=torch.squeeze(out_code[ib])
        out_code2=out_code1.detach().numpy()
        d={}
        i=0
        for code in out_code2:
            d[i]=code
            i=i+1
        ...
        i=0
        for key in want:
            img=x[ib][key[0]].detach().numpy() #[32,32]
            img=1.0/(1+np.exp(-1*img))
            img=np.round(img*255)
            filename='F:/postgraduate/little paper2.1/DM-GAN/models/{}-{}-{}.jpg'.format(ib,i,key[0])
...
class INIT_STAGE_G(nn.Module):
    ...
    def forward(self, z_code, c_code,imgs):
        """
        :param imgs:batch x 64 x 32 x 32
        ...
        # state size ngf/8 x 32 x 32[8, 128, 32, 32]
        out_code32 = self.upsample3(out_code)
        #引入人脸模板并进行特征融合
        out_code32_img=self.ACM(out_code32,imgs)
        out_code32_img=self.residual(out_code32_img)
        # state size ngf/16 x 64 x 64
        out_code64 = self.upsample4(out_code32_img)
...
class Memory(nn.Module):
    ...
    def forward(self, input, context_key, content_value):#
        ...
        target = input.view(batch_size, -1, queryL) #将input（batch x idf x ih x iw）->（batch x idf x queryL）合并了宽高
        targetT = torch.transpose(target, 1, 2).contiguous() #（batch x idf x queryL）->（batch x queryL x idf）
        sourceT = context_key

        # Get weight
        # (batch x queryL x idf)(batch x idf x sourceL)-->batch x queryL x sourceL
        weight = torch.bmm(targetT, sourceT)#键寻址
        ...
        weightedContext = torch.bmm(content_value, weight)  #值读取
...
class NEXT_STAGE_G(nn.Module):
    ...
    def forward(self, h_code, c_code, word_embs, mask, cap_lens):
        ...
        """
        # Memory Writing
        word_embs_T = torch.transpose(word_embs, 1, 2).contiguous() #batch x cdf x sourceL->batch x sourceL x cdf
        h_code_avg = self.avg(h_code).detach() #等大小avg pooling，相当于求平均 batch x idf x ih x iw->batch x idf x 1 x 1
        h_code_avg = h_code_avg.squeeze(3) #delete dim 4 batch x idf x 1 x 1->batch x idf x 1
        h_code_avg_T = torch.transpose(h_code_avg, 1, 2).contiguous() #交换2，3维 batch x idf x 1->batch x 1 x idf
        gate1 = torch.transpose(self.A(word_embs_T), 1, 2).contiguous() #A*W_i batch x sourceL x cdf->batch x cdf x sourceL
        gate2 = self.B(h_code_avg_T).repeat(1, 1, word_embs.size(2)) #B*sum(r_i)/N batch x 1 x idf->batch x 1 x idf*sourceL
        ...
        #torch.Size([bs, 128, 64, 64])
        # self.ACM(h_code_new,imgs) #[bs, 128, 64, 64] [batch,64,64,64]
        out_code = self.residual(h_code_new)
        #torch.Size([bs, 128, 64, 64])

        # state size ngf/2 x 2in_size x 2in_size
        out_code = self.upsample(out_code)
        #torch.Size([bs, 64, 128, 128])
        return out_code, att

class FINAL_STAGE_G(nn.Module):
    def __init__(self, ngf, nef, ncf, size):
        super(FINAL_STAGE_G, self).__init__()
        self.gf_dim = ngf
        self.ef_dim = nef
        self.cf_dim = ncf
        self.num_residual = cfg.GAN.R_NUM
...
class G_NET(nn.Module):
    def __init__(self):
        ...
        if cfg.TREE.BRANCH_NUM > 0:
            ...
            self.imgUpSample2= imgUpBlock(nef, ngf)
        # gf x 64 x 64
        if cfg.TREE.BRANCH_NUM > 1:
            self.h_net2 = NEXT_STAGE_G(ngf, nef, ncf, 64)
            self.img_net2 = GET_IMAGE_G(ngf)
        if cfg.TREE.BRANCH_NUM > 2:
            self.h_net3 = FINAL_STAGE_G(ngf, nef, ncf, 128)
            self.img_net3 = GET_IMAGE_G(ngf)

    def forward(self, z_code, sent_emb, word_embs, mask, cap_lens,region_features):
        """
            :param z_code: batch x cfg.GAN.Z_DIM
            :param sent_emb: batch x cfg.TEXT.EMBEDDING_DIM
            :param word_embs: batch x cdf x seq_len
            :param mask: batch x seq_len
            :region:batch x 256 x 17 x 17
            :return:
        """
        fake_imgs = []
        att_maps = []
        c_code, mu, logvar = self.ca_net(sent_emb)
        img_code32 = self.imgUpSample1(region_features)#img_code32:[8, 64, 32, 32]
        h_code1 = self.h_net1(z_code, c_code,img_code32)
        fake_img1 = self.img_net1(h_code1)
        fake_imgs.append(fake_img1)
        # img_code64 = self.imgUpSample2(img_code32)
        # print("img_code64")
        # print(img_code64.shape)
